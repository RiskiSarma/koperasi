...
if (empty($jml_data)) {
    ...
    while ($data = mysqli_fetch_array($query)) {
        ...
        $jumlah_pinjaman1 = $jumlah_pinjaman1 ?? 0;
        $nilai_angsuran = $nilai_angsuran ?? 0;
        $estimasi_jasa = $estimasi_jasa ?? 0;
        ...
        if ($status == "Pending") {
            ...
            $LabelStatus = '<span class="badge bg-success">Lunas</span>';
        } elseif ($status == "Macet") {
        ...
        $email = !empty($DataAnggota['email']) ? $DataAnggota['email'] : "No Email";

        //Buka data akses
        $QryDetailAkses = mysqli_query($Conn, "SELECT * FROM akses WHERE id_akses='$id_akses'") or die(mysqli_error($Conn));
        $DataDetailAkses = mysqli_fetch_array($QryDetailAkses);
        $nama_akses = !empty($DataDetailAkses['nama_akses']) ? $DataDetailAkses['nama_akses'] : '<span class="text-danger">None</span>';

        //Cek Jurnal
        $CekJurnal = mysqli_num_rows(mysqli_query($Conn, "SELECT * FROM jurnal WHERE id_pinjaman='$id_pinjaman'"));
        $LabelJurnal = !empty($CekJurnal) ? '<span class="text-success" title="Tersedia Jurnal"><i class="bi bi-check-circle"></i> Jurnal</span>' : '<span class="text-dark" title="Tidak Tersedia Jurnal"><i class="bi bi-x"></i> None</span>';
        ...
        if (!empty($JumlahAngsuranBerjalan)) {
            ...
            $JumlahAngsuran1 = $SumAngsuran['jumlah'] ?? 0;
            ...
            $JumlahAngsuranJasa = $SumAngsuranJasa['jumlah'] ?? 0;
